...
Главный дашборд автоматически редактируется каждые `STATUS_REFRESH_SECONDS` секунд (по умолчанию 10): GPU, VRAM, температура и состояние сервисов всегда обновляются в одном сообщении. Значение `0` отключает автообновление.
